...
def curated_must_pass() -> List[Case]:
    ...
    return [
        ...
        Case("must_28_outlook_headers_only", "unparseable_control",
             ...
             expected=[],
             must_pass=True),
        # Exhaustive day-list separator coverage. Each case shares ONE time
        # window across a list of days using a different separator. All must
        # yield the same 3-window schedule.
        Case("must_29_oxford_comma_and", "multi_day_list",
             "Mon, Tue, and Wed 6am-4pm",
             expected=[(0, 6, 16), (1, 6, 16), (2, 6, 16)],
             must_pass=True),
        Case("must_30_semicolon_shared_time", "multi_day_list",
             "Mon; Tue; Wed 0600-1600",
             expected=[(0, 6, 16), (1, 6, 16), (2, 6, 16)],
             must_pass=True),
        Case("must_31_backslash_sep", "multi_day_list",
             "Mon\\Tue\\Wed 0600-1600",
             expected=[(0, 6, 16), (1, 6, 16), (2, 6, 16)],
             must_pass=True),
        Case("must_32_pipe_sep", "multi_day_list",
             "Mon|Tue|Wed 0600-1600",
             expected=[(0, 6, 16), (1, 6, 16), (2, 6, 16)],
             must_pass=True),
        Case("must_33_plus_sep", "multi_day_list",
             "Mon+Tue+Wed 0600-1600",
             expected=[(0, 6, 16), (1, 6, 16), (2, 6, 16)],
             must_pass=True),
        Case("must_34_mixed_comma_and_amp", "multi_day_list",
             "Mon, Tue & Wed 6am-4pm",
             expected=[(0, 6, 16), (1, 6, 16), (2, 6, 16)],
             must_pass=True),
        Case("must_35_mixed_comma_oxford_amp", "multi_day_list",
             "Mon, Tue, & Wed 6am-4pm",
             expected=[(0, 6, 16), (1, 6, 16), (2, 6, 16)],
             must_pass=True),
        Case("must_36_space_ampersand", "multi_day_list",
             "Monday & Tuesday & Wednesday 6am-4pm",
             expected=[(0, 6, 16), (1, 6, 16), (2, 6, 16)],
             must_pass=True),
        Case("must_37_semicolon_plus_and", "multi_day_list",
...
def run_combined(case: Case, api_key: str) -> CaseResult:
    try:
        ...
        entries, confidence, notes, _method = _silent(
